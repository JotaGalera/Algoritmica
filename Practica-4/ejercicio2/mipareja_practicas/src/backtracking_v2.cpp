...
bool findItem(vector<int> v, int x){
    if(std::find(v.begin(), v.end(), x) != v.end()) { /* v contains x */
        return true;
...
vector<vector<int> > personasLibres(const Apermutacion &P, const vector<vector<int> > &matriz_stl){
    vector<vector<int> > libres;
    vector<int> elegidos;
    Apermutacion::const_iterator it;
    for(it = P.begin(); it != P.end(); ++it)
        elegidos.push_back(*it-1);

    for(int i = 0 ; i < matriz_stl.size(); i++)
        if(!findItem(elegidos,i))
            libres.push_back(matriz_stl[i]);

    return libres;
}

int estimacionAfinidad(const vector<vector<int> > &matriz_stl){
    /*
        d_estiama = d_actual + CotaI(indiviuosLibres)
        
    */
    int cotaS = numeric_limits<int>::max();
    int d_estimada = 0;
    int pA, pB, cotaI;

    vector <pair<int, int> > parejas;
    for (pA = 0; pA < matriz_stl.size(); ++pA) {
        cotaI = cotaS;
        for (int j = 0; j < matriz_stl[pA].size(); ++j) {
            if (matriz_stl[pA][j] < cotaI) {
                cotaI = matriz_stl[pA][j];
                pB = j;
            }
        }
        parejas.push_back(make_pair(pA, pB));
    }

    for (int k = 0; k < parejas.size(); ++k)
        d_estimada += matriz_stl[parejas[k].first][parejas[k].second];
...
int backtracking(int n, Apermutacion &ab, const vector<vector<int> > &matriz_stl){
   ...
   while(seguir){
    ...
    if (P.GetLevel() == n-1) {
        ...
        vector<vector<int> > personas_libres = personasLibres(P, matriz_stl);
        int afi_estimada = estimacionAfinidad(personas_libres);
        
        if (afi_estimada + afinidad_actual >= best_afinidad) {
            seguir = P.Backtracking();
        }
        else {
            seguir = P.GeneraSiguienteProfundidad();
...
int main(int argc, char *argv[]){
   ...
   //Calcularemos la matriz de discrepancia
    ...
    cout << "\nDiscrepancia con Estimacion Backtraking: " << discrepancia_backtracking << endl << endl;
    solucion(P);
